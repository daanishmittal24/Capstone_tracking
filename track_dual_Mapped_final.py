...
# ZOOMCAM_HEIGHT = 360
# UDP_PORT = 5000
# OUT_FPS = 30  # Increased from 20 for smoother preview and faster detection
...
# WHEEL_TARGET_TOLERANCE = 1.0  # Degrees within which we consider the wheel on target (increased from 0.3 to prevent jitter)

# # ZoomCam fine control tuning
# ZOOMCAM_FINE_KP = 12.0  # Degrees of correction per normalized Y error - REDUCED to let mapping work better
# ZOOMCAM_FINE_MAX_CORR = 2.0  # Clamp fine corrections - REDUCED to prevent fighting with mapping
# ZOOMCAM_TARGET_SMOOTHING = 0.65  # Exponential smoothing factor for wheel target (0-1) - MORE smoothing for stability
# ZOOMCAM_FINE_I_GAIN = 3.5  # Integral gain - REDUCED to prevent overshoot
# ZOOMCAM_FINE_I_MAX = 4.0  # Clamp integral bias - REDUCED for safety
# ZOOMCAM_FINE_I_DECAY_SECONDS = 2.5  # Time constant for letting bias drift back toward zero without a face
# ZOOMCAM_CENTER_DEAD_ZONE = 0.06  # Normalized Y dead zone (±6% = ±22px at 360p) - INCREASED for stability
# ZOOMCAM_TARGET_CHANGE_THRESHOLD = 0.8  # Don't update motor if target changes less than this (degrees) - INCREASED to ignore small fluctuations
# ZOOMCAM_MIN_MOVE_INTERVAL = 0.5  # Minimum seconds between motor commands - INCREASED to prevent rapid oscillation

# # Variable distance handling (for blur/close objects)
# BLUR_VARIANCE_THRESHOLD = 80.0  # Laplacian variance threshold - below this = blurry (was 100)
...
# VERTICAL_MAPPING_SMOOTH_WINDOW = 5  # Moving-average window (will be forced odd)
# VERTICAL_MAPPING_ENFORCE_MONOTONIC = True  # Keep mapping strictly monotonic
...
#     Strategy: Group samples by wheel angle, take median Y position for each angle,
#     then create monotonic Y→Wheel mapping.
#     """
#     def __init__(self, samples):
#         if not samples:
#             raise ValueError('Vertical mapping requires at least one sample')
        
#         # Group Y positions by wheel angle
#         per_wheel_angle = {}
#         for item in samples:
#             wheel_deg = float(item['wheel_degrees'])
#             y_norm = float(item['arducam_y_normalized'])
#             per_wheel_angle.setdefault(wheel_deg, []).append(y_norm)
        
#         # Take median Y position for each wheel angle
#         wheel_to_y = []
#         for wheel_deg in sorted(per_wheel_angle.keys()):
#             y_values = per_wheel_angle[wheel_deg]
#             median_y = float(np.median(y_values))
#             wheel_to_y.append((wheel_deg, median_y))
        
#         if len(wheel_to_y) < 2:
#             raise ValueError('Need at least 2 distinct wheel angles for mapping')
        
#         # Extract arrays
#         wheel_angles = np.array([p[0] for p in wheel_to_y], dtype=np.float32)
#         y_positions = np.array([p[1] for p in wheel_to_y], dtype=np.float32)
        
#         # Enforce monotonic: Y should increase as wheel angle increases
#         # (face appears lower in frame as camera tilts up)
#         if VERTICAL_MAPPING_ENFORCE_MONOTONIC and len(y_positions) > 1:
#             # Force Y positions to be monotonically increasing with wheel angle
#             y_positions = np.maximum.accumulate(y_positions)
        
#         # Apply smoothing to reduce noise
#         window = max(1, int(VERTICAL_MAPPING_SMOOTH_WINDOW))
#         if window % 2 == 0:
#             window += 1
#         if window > 1 and len(y_positions) >= 3:
#             pad = window // 2
#             kernel = np.ones(window, dtype=np.float32) / window
#             padded = np.pad(y_positions, (pad, pad), mode='edge')
#             y_positions = np.convolve(padded, kernel, mode='valid')
        
#         # Store as (Y_norm, Wheel_deg) pairs for lookup
#         # Invert the mapping: we want Y → Wheel, so sort by Y
#         y_wheel_pairs = list(zip(y_positions.tolist(), wheel_angles.tolist()))
#         y_wheel_pairs.sort(key=lambda p: p[0])
        
#         # Remove duplicate Y values (keep first occurrence)
#         unique_pairs = []
#         last_y = None
#         for y, w in y_wheel_pairs:
#             if last_y is None or abs(y - last_y) > 1e-4:
#                 unique_pairs.append((y, w))
#                 last_y = y
...
#         # Debug output
#         print(f'[VerticalMapping] Processed {len(samples)} samples → {len(unique_pairs)} unique Y→Wheel points')
#         print(f'[VerticalMapping] Y range: {self.y_min:.3f} to {self.y_max:.3f}')
#         print(f'[VerticalMapping] Wheel range: {self.samples[0][1]:+.1f}° to {self.samples[-1][1]:+.1f}°')
...
#         """Interpolate wheel angle from normalized Y position."""
#         if not self.samples:
#             return 0.0
...
#     """Move the wheel axis in WHEEL_STEPWISE_INCREMENT° increments so we can observe hall events mid-sweep.

#     Returns (hall_triggered, last_direction, last_status).
#     """
#     steps = int(abs(total_degrees) // WHEEL_STEPWISE_INCREMENT)
#     remainder = abs(total_degrees) % WHEEL_STEPWISE_INCREMENT
#     if steps == 0 and remainder == 0:
#         return False, 0, None
...
#         status = send_wheel_delta(direction * WHEEL_STEPWISE_INCREMENT, serial_lock)
#         if status:
#             last_status = status
...
#     """Move the base axis in BASE_STEPWISE_INCREMENT° increments so we can observe hall events mid-sweep.

#     Returns (hall_triggered, last_direction, last_status).
#     """
#     steps = int(abs(total_degrees) // BASE_STEPWISE_INCREMENT)
#     remainder = abs(total_degrees) % BASE_STEPWISE_INCREMENT
#     if steps == 0 and remainder == 0:
#         return False, 0, None
...
#     for _ in range(steps):
#         status = send_base_delta(direction * BASE_STEPWISE_INCREMENT, serial_lock)
#         if status:
#             last_status = status
...
#         time.sleep(HOMING_POST_MOVE_DELAY)
#         if hall_triggered:
#             print(f'[Homing] ✅ Confirmed with {step}° step (immediate stop)')
#             return True
#     print('[Homing] ⚠️  Fine approach failed to confirm hall sensor')
#     return False
...
# def arducam_process(serial_lock, stop_event, shared_face_state, motion_state, mapping_points):
#     """
...
#         print(f'[Arducam] ✅ Loaded vertical mapping with {len(vertical_mapping.samples)} samples')
#     except Exception as exc:
#         print(f'[Arducam] ❌ Mapping unavailable: {exc}')
#         stop_event.set()
#         return
...
#             controls={"FrameRate": 30}  # Request 30 FPS
#         )
#         picam2.configure(config)
#         picam2.start()
#         picam2.set_controls({"AfMode": 2, "AfTrigger": 0})
#         time.sleep(1)
#         print('[Arducam] ✅ Camera started at 30 FPS')
#     except Exception as e:
...
#     current_fps = 0
#     last_send_time = 0.0
#     predicted_wheel_deg = None
#     detect_frame_skip = 1  # Run face detection every frame for Arducam (wider FOV needs less skipping)
#     detect_frame_counter = 0
#     last_faces = None  # Cache last detection
...
#             # === FACE DETECTION (for horizontal tracking + position sharing with frame skip) ===
#             detect_frame_counter += 1
#             if detect_frame_counter >= detect_frame_skip:
#                 detect_frame_counter = 0
#                 _, last_faces = face_detector.detect(frame_bgr)
...
#                 predicted_wheel_deg = vertical_mapping.lookup(face_y_normalized)
#                 timestamp = time.time()
#                 shared_face_state['arducam_face_y'] = face_y_normalized
#                 shared_face_state['arducam_cy'] = cy
#                 shared_face_state['arducam_height'] = h
#                 shared_face_state['timestamp'] = timestamp
#                 shared_face_state['target_wheel_deg'] = predicted_wheel_deg
#                 shared_face_state['target_wheel_timestamp'] = timestamp
...
#                 cv2.putText(disp, f'Wheel prediction: {predicted_wheel_deg:+.1f}°', (dcx+10, dcy+30),
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (128, 200, 255), 2)
#             else:
#                 disp = cv2.resize(frame_bgr, (ARDUCAM_DISPLAY_WIDTH, ARDUCAM_DISPLAY_HEIGHT))
#                 scale_x = ARDUCAM_DISPLAY_WIDTH / w
#                 scale_y = ARDUCAM_DISPLAY_HEIGHT / h
...
#                 shared_face_state['target_wheel_deg'] = None
#                 shared_face_state['target_wheel_timestamp'] = 0
#                 predicted_wheel_deg = None
...
#             pred_status = f'{predicted_wheel_deg:+.1f}°' if predicted_wheel_deg is not None else 'n/a'
#             cv2.putText(disp, f'Wheel prediction: {pred_status}', (10, 95),
...
#     smoothed_target_deg = None
#     fine_bias_deg = 0.0
#     last_loop_time = time.time()
#     detect_frame_skip = 1  # Run face detection every frame for maximum responsiveness (1=every frame, 2=every other)
#     detect_frame_counter = 0
#     last_zoomcam_face_y_norm = 0.5
#     last_zoomcam_has_face = False
#     last_face_box = None  # Store (x, y, w, h) for drawing
    
#     # Asynchronous motor control
#     motor_queue = ThreadQueue(maxsize=2)
#     motor_stop = threading.Event()
#     motor_target_deg = [None]  # Mutable container for thread sharing
#     motor_last_move_time = [0.0]
    
#     def motor_control_thread():
#         """Background thread that handles wheel motor commands without blocking frame processing."""
#         nonlocal estimated_wheel_deg
#         last_send = 0.0
#         consecutive_failures = 0
        
#         while not motor_stop.is_set() and not stop_event.is_set():
#             target = motor_target_deg[0]
#             if target is None:
#                 time.sleep(0.02)
#                 consecutive_failures = 0
#                 continue
            
#             # Refresh estimate from shared state
#             estimated_wheel_deg = get_shared_wheel_deg(estimated_wheel_deg)
#             delta = target - estimated_wheel_deg
...
#             # Enforce minimum interval between moves to prevent oscillation
#             now = time.time()
#             elapsed_since_last = now - last_send
#             if elapsed_since_last < ZOOMCAM_MIN_MOVE_INTERVAL:
#                 time.sleep(0.02)
#                 continue
...
#             last_send = now
#             motor_last_move_time[0] = now
            
#             if status and status.get('wheel_deg') is not None:
#                 estimated_wheel_deg = status['wheel_deg']
#                 consecutive_failures = 0
#                 print(f'[ZoomCam] Target {target:+.1f}°, move {step:+d}° → actual {estimated_wheel_deg:+.1f}°')
#             else:
#                 # Fallback: use expected position
#                 estimated_wheel_deg += step
#                 set_shared_wheel_deg(estimated_wheel_deg)
#                 consecutive_failures += 1
#                 print(f'[ZoomCam] Target {target:+.1f}°, move {step:+d}° → est {estimated_wheel_deg:+.1f}° (no status, attempt {consecutive_failures})')
                
#                 # If too many consecutive failures, request explicit status
#                 if consecutive_failures >= 3:
#                     fallback_status = request_status(serial_lock)
#                     if fallback_status and fallback_status.get('wheel_deg') is not None:
#                         estimated_wheel_deg = fallback_status['wheel_deg']
#                         print(f'[ZoomCam] ✅ Recovered status: {estimated_wheel_deg:+.1f}°')
#                         consecutive_failures = 0
            
#             time.sleep(SETTLING_DELAY)  # Motor settling happens in background thread
    
#     motor_thread = threading.Thread(target=motor_control_thread, daemon=True)
#     motor_thread.start()

#     print('[ZoomCam] 🎯 Mapping Arducam face position to wheel angle')
...
#             h, w = frame.shape[:2]
#             loop_now = time.time()
#             dt = max(1e-3, loop_now - last_loop_time)
...
#             predicted_wheel = shared_face_state.get('target_wheel_deg')
#             predicted_timestamp = shared_face_state.get('target_wheel_timestamp', 0)
            
#             # Check if Arducam data is fresh (within last 0.5 seconds)
#             data_is_fresh = (loop_now - arducam_timestamp) < 0.5
#             prediction_is_fresh = (loop_now - predicted_timestamp) < 0.5
#             target_wheel_deg = None
#             target_from_prediction = False
            
#             # === DETECT FACE IN ZOOMCAM (with frame skipping for performance) ===
#             detect_frame_counter += 1
#             if detect_frame_counter >= detect_frame_skip:
#                 detect_frame_counter = 0
...
#                     last_zoomcam_has_face = True
#                     last_zoomcam_face_y_norm = cy / h
#                     last_face_box = (x, y, box_w, box_h, cx, cy)
#                 else:
#                     last_zoomcam_has_face = False
#                     last_face_box = None
            
#             # Use cached detection results
#             zoomcam_has_face = last_zoomcam_has_face
#             zoomcam_face_y_norm = last_zoomcam_face_y_norm
            
#             # Draw face bounding box if detected
#             if zoomcam_has_face and last_face_box is not None:
#                 x, y, box_w, box_h, cx, cy = last_face_box
#                 cv2.rectangle(frame, (x, y), (x + box_w, y + box_h), (0, 255, 0), 2)
#                 cv2.circle(frame, (cx, cy), 5, (0, 0, 255), -1)
#                 cv2.putText(frame, f'ZoomCam Y: {zoomcam_face_y_norm:.2f}', (cx + 10, cy - 10),
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
            
#             # === MAP ARDUCAM FACE TO WHEEL TARGET ===
#             if prediction_is_fresh and predicted_wheel is not None:
#                 target_wheel_deg = float(predicted_wheel)
#                 target_from_prediction = True
#             elif arducam_has_face and data_is_fresh:
#                 target_wheel_deg = vertical_mapping.lookup(arducam_face_y_norm)
#                 arducam_y_in_frame = int(arducam_face_y_norm * h)
#                 cv2.line(frame, (0, arducam_y_in_frame), (w, arducam_y_in_frame), (255, 128, 0), 2)
#                 cv2.putText(frame, f'Arducam Y: {arducam_face_y_norm:.3f}', (10, arducam_y_in_frame-10),
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 128, 0), 2)
#                 cv2.putText(frame, f'Mapped: {target_wheel_deg:+.1f}°', (10, arducam_y_in_frame+15),
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (128, 255, 0), 2)

#             fine_correction = 0.0
#             adaptive_bias = 0.0
#             if target_wheel_deg is not None and zoomcam_has_face:
#                 center_error = 0.5 - zoomcam_face_y_norm  # positive if face appears low (need to tilt up)
                
#                 # Apply dead zone: if face is already well-centered, don't apply fine correction
#                 if abs(center_error) > ZOOMCAM_CENTER_DEAD_ZONE:
#                     # Face is off-center - apply proportional correction and update integral
#                     fine_correction = np.clip(center_error * ZOOMCAM_FINE_KP,
#                                               -ZOOMCAM_FINE_MAX_CORR,
#                                               ZOOMCAM_FINE_MAX_CORR)
#                     target_wheel_deg += fine_correction

#                     # Update integral bias to learn persistent offsets
#                     fine_bias_deg += float(center_error) * ZOOMCAM_FINE_I_GAIN * dt
#                     fine_bias_deg = float(np.clip(fine_bias_deg, -ZOOMCAM_FINE_I_MAX, ZOOMCAM_FINE_I_MAX))
#                 else:
#                     # Face is centered - no new P correction, but maintain learned I bias
#                     fine_correction = 0.0
                
#                 # Always apply learned integral bias (even when centered)
#                 adaptive_bias = fine_bias_deg
#                 target_wheel_deg += adaptive_bias
#             else:
#                 # No face detected - decay the integral bias
#                 if ZOOMCAM_FINE_I_DECAY_SECONDS > 0:
#                     decay = math.exp(-dt / ZOOMCAM_FINE_I_DECAY_SECONDS)
#                     fine_bias_deg *= decay
#                 else:
#                     fine_bias_deg = 0.0

#             if target_wheel_deg is not None:
#                 if smoothed_target_deg is None:
#                     smoothed_target_deg = target_wheel_deg
#                 else:
#                     smoothed_target_deg += ZOOMCAM_TARGET_SMOOTHING * (target_wheel_deg - smoothed_target_deg)
                
#                 # Only update motor target if change is significant (prevents jitter)
#                 current_motor_target = motor_target_deg[0]
#                 target_delta = abs(smoothed_target_deg - current_motor_target) if current_motor_target is not None else float('inf')
                
#                 if current_motor_target is None or target_delta > ZOOMCAM_TARGET_CHANGE_THRESHOLD:
#                     motor_target_deg[0] = smoothed_target_deg
#                     print(f'[ZoomCam] 🎯 New motor target: {smoothed_target_deg:+.1f}° (delta: {target_delta:.2f}°)')
                
#                 label = 'prediction' if target_from_prediction else 'mapping'
#                 cv2.putText(frame, f'Target wheel ({label}): {target_wheel_deg:+.1f}°', (10, 100),
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
#                 if zoomcam_has_face:
#                     center_err_val = abs(0.5 - zoomcam_face_y_norm)
#                     cv2.putText(frame, f'Fine corr: {fine_correction:+.2f}°', (10, 160),
#                                 cv2.FONT_HERSHEY_SIMPLEX, 0.5, (128, 200, 200), 1)
#                     cv2.putText(frame, f'I-bias: {adaptive_bias:+.2f}°', (10, 180),
#                                 cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 128), 1)
#                     cv2.putText(frame, f'Error: {center_err_val:.3f}', (10, 200),
#                                 cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
#                     # Show if face is in dead zone (centered)
#                     if center_err_val <= ZOOMCAM_CENTER_DEAD_ZONE:
#                         cv2.putText(frame, '✓ CENTERED', (10, 220),
#                                     cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
#                     else:
#                         cv2.putText(frame, f'ADJUSTING ({center_err_val:.3f})', (10, 220),
#                                     cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 165, 255), 1)
#             else:
#                 smoothed_target_deg = None
#                 motor_target_deg[0] = None
#             estimated_wheel_deg = get_shared_wheel_deg(estimated_wheel_deg)
#             cv2.putText(frame, f'Estimated wheel: {estimated_wheel_deg:+.1f}°', (10, 120),
#                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 200), 2)
            
#             # UI overlay
#             cv2.putText(frame, 'ZOOMCAM - Mapping-based Vertical Control', (10, 25),
...
#             if prediction_is_fresh and predicted_wheel is not None:
#                 status = 'Using Arducam prediction'
#             elif arducam_has_face and data_is_fresh:
#                 status = 'Mapping control active'
...
#             pred_text = 'n/a'
#             if prediction_is_fresh and predicted_wheel is not None:
#                 pred_text = f'{predicted_wheel:+.1f}°'
#             cv2.putText(frame, f'Arducam prediction: {pred_text}', (10, 140),
#                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 128, 255), 1)
...
#     print('  - Arducam: Wide FOV - detects face, controls horizontal, shares Y position')
#     print('  - ZoomCam: Narrow FOV - uses mapping to align vertical tilt')
#     print()
#     print('✅ Both axes home to their Hall sensors + load mapping before start')
...
#                         args=(serial_lock, stop_event, shared_face_state, shared_motion_state, mapping_payload),
#                         name='Arducam')
#     p_zoomcam = Process(
...
def zoomcam_process(serial_lock, stop_event, shared_face_state, motion_state, mapping_points, initial_wheel_deg):
    ...
    print('[ZoomCam] Processing in headless low-CPU mode')
