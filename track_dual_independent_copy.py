...
WHEEL_TARGET_TOLERANCE = 0.3  # Degrees within which we consider the wheel on target
...
def refine_hall_alignment(serial_lock, last_move_direction):
    approach_dir = 1 if last_move_direction >= 0 else -1
    fine_steps = [5, 2, 1]
    print('[Homing] Refining hall alignment...')
    send_wheel_delta(-approach_dir * 5, serial_lock)
    time.sleep(HOMING_POST_MOVE_DELAY)
    for step in fine_steps:
        send_wheel_delta(approach_dir * step, serial_lock)
        time.sleep(HOMING_POST_MOVE_DELAY)
        status = request_status(serial_lock)
        if status and status.get('hall_wheel', 0) == 1:
            print(f'[Homing] ✅ Confirmed with {step}° step')
            return True
    print('[Homing] ⚠️  Fine approach failed to confirm hall sensor')
    return False


def find_hall_home(serial_lock):
    print('\n[Homing] Starting Hall sensor homing sequence...')
    status = request_status(serial_lock)
    if status and status.get('hall_wheel', 0) == 1:
        print('[Homing] ⚠️  Already at Hall sensor. Moving away first...')
    ...
    for idx, step in enumerate(HOMING_SWEEP_STEPS, start=1):
        status = request_status(serial_lock)
        if status is None:
            print('[Homing] ⚠️  No status response')
            continue
        ...
        if hall == 1:
            ...
            refine_hall_alignment(serial_lock, approach)
            zero_encoders(serial_lock)
            time.sleep(HOMING_SETTLE_DELAY)
            return True
        move = direction * step
        print(f'[Homing] Step {idx}/{len(HOMING_SWEEP_STEPS)}: Moving {move:+d}°')
        send_wheel_delta(move, serial_lock)
        time.sleep(HOMING_POST_MOVE_DELAY)
        last_move_dir = 1 if move > 0 else -1
        direction *= -1

    print('[Homing] ⚠️  Hall sensor not found. Zeroing current position...')
    zero_encoders(serial_lock)
    time.sleep(HOMING_SETTLE_DELAY)
    return False
...
def zoomcam_process(serial_lock, stop_event, shared_face_state, motion_state, mapping_points, initial_wheel_deg):
    ...
    def drive_wheel_to_target(target_deg):
        nonlocal last_send_time, estimated_wheel_deg
        ...
        print(f'[ZoomCam] Target {target_deg:+.1f}°, move {step:+d}° → est {estimated_wheel_deg:+.1f}°')
        time.sleep(SETTLING_DELAY)
    ...
    try:
        while not stop_event.is_set():
            ...
            if arducam_has_face and data_is_fresh:
                ...
                drive_wheel_to_target(target_wheel_deg)
                ...
                if target_wheel_deg is not None:
                    cv2.putText(frame, f'Target wheel: {target_wheel_deg:+.1f}°', (10, 100),
...
def main():
    ...
    home_found = find_hall_home(serial_lock)
    if home_found:
